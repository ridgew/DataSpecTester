...
namespace Gwsoft.DataSpec2
{
    ...
    {
        /// <summary>
        /// 初始化一个 <see cref="SpecExpression"/> class 实例。
        /// </summary>
        /// <param name="expDefine">表达式定义</param>
        public SpecExpression(string expDefine)
        {
            expressionRaw = expDefine;
        }

        /*
         { ESP_SuccessFlag == 1020 }
         { ESP_ParamsCount > 18 } 
         { ESP_PackageIndex > 0 && ESP_PackageLength > 0 }
         {!(ESP_SuccessFlag == 1120)}
        */
        string expressionRaw = null;

        /// <summary>
        /// 判断在当前上下文是否满足传输的条件
        /// </summary>
        /// <param name="context">字节序列上下文</param>
        public bool IsPass(StreamContext context)
        {
            using (ExpressionParse parse = new ExpressionParse(expressionRaw, new ContextDependency(context.ContextObjectDictionary)))
            {
                return Convert.ToBoolean(parse.ComputeExpression());
            }
        }

        class ContextDependency : ExpressionParse.IResourceDependency
        {

            /// <summary>
            /// 初始化一个 <see cref="ContextDependency"/> class 实例。
            /// </summary>
            /// <param name="ctxtDict">The CTXT dict.</param>
            public ContextDependency(Dictionary<string, object> ctxtDict)
            {
                innerDict = ctxtDict;
            }
            ...
            {
                return innerDict.ContainsKey(x);
            }
            ...
            {
                get { return typeof(ContextDependency).Name; }
            }
    ...
    /// <summary>
    /// 简易表达式解析实现
    /// (by Ridge Wong: http://fmq.googlecode.com/svn-history/r230/trunk/SiteMagic/Webot.SiteMagic/Tags/TagParse.cs);
    /// </summary>
    public class ExpressionParse : IDisposable
    {
        ...
        /// g: {#$ [3,[4,5]][1][1] + 5 $#} = 10
        /// </remarks>
        /// <param name="tagdef">表达式内容定义体</param>
        public ExpressionParse(string tagdef) : this(tagdef, null) { }

        /// <summary>
        /// 初始化一个 <see cref="ExpressionParse"/> class 实例。
        /// </summary>
        /// <param name="tagdef">The tagdef.</param>
        /// <param name="res">The res.</param>
        public ExpressionParse(string tagdef, IResourceDependency res)
        {
            if (!tagdef.StartsWith("{") || !tagdef.EndsWith("}"))
            {
                throw new InvalidProgramException("表达式定义错误，请使用{}包含完整的表达式定义！");
            }
            else
            {
                SetResourceDependency(res);
                this._tagDef = tagdef;
                initializeExpression(tagdef.Substring(1, tagdef.Length - 2).Trim());
            }
        }

        List<InnerExpression> innerExp = null;

        void initializeExpression(string tagBody)
        {
            innerExp = GetInnerExpressions(tagBody, GetResourceDependency());
        }
        ...
        public static List<InnerExpression> GetInnerExpressions(string ExpPaper, IResourceDependency Res)
        {
            ...
            while (cursor < lenT)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        if (strTemp.Length > 0)
                        {
                            expList.Add(new InnerExpression(strTemp, Res));
                        }
            ...
            //{
            //    Console.WriteLine(string.Format("定义:{0}, Entity:{1}, OP:{2}, Dependency:{3} ", ext.TagDefinition, ext.IsEntity, ext.IsOperator(), ext.IsDependencyExpression));
            //}
            //#endregion
            return expList;
        }
        ...
        {
            object objResult = ComputeExpression();
        ...
        public object ComputeExpression()
        {
            ...
            if (innerExp != null)
            {
                strRet = ExecExpressions(innerExp);
            }
            return strRet;
        }
        ...
        public interface IExpression
        {
            ...
            /// 是否有依赖性
            /// </summary>
            bool IsDependencyExpression { get; }
        ...
        /// 内部表达式
        /// </summary>
        public class InnerExpression : IExpression
        {
            /// <summary>
            ...
            {
                if (IsFunction == true && _equalFun != null)
                {
                    _equalFun.SetResourceDependency(GetResourceDependency());
                    return _equalFun.Execute();
                }
                else
                {
                    ...
                    {
                        if (IsOperator()) return strResult;

                        if (string.Equals(strResult, "true", StringComparison.InvariantCultureIgnoreCase)
                            || string.Equals(strResult, "false", StringComparison.InvariantCultureIgnoreCase))
                        {
                            return strResult;
                        }
                        else
                        {
                            IResourceDependency res = GetResourceDependency();
                            if (res != null && res.IsDefined(strResult))
                            {
                                return res.GetDefinition(strResult);
                            }
                            return strResult;
                        }
                    }
            ...
            /// </summary>
            /// <value></value>
            public bool IsDependencyExpression
            {
        ...
        /// 内部参数
        /// </summary>
        public class InnerParam : IExpression
        {
            /// <summary>
            ...
            public object GetValue()
            {
                ...
                else
                {
                    ...
                    #region 布尔及数字型
                    if (string.Compare(ParamDefine, "true", true) == 0)
                    {
                        return true;
                    }
                    else if (string.Compare(ParamDefine, "false", true) == 0)
                    {
                        return false;
                    }
                    else
                    {
                        ...
                        {
                            if (Res != null && Res.IsDefined(ParamDefine))
                            {
                                return Res.GetDefinition(ParamDefine);
                            }
                            else
                            {
                                return ParamDefine;
        ...
        /// 内部函数
        /// </summary>
        public class InnerFunction : IExpression
        {
            /// <summary>
            /// 内部函数调用实例
            /// </summary>
            ...
            public object Execute()
            {
                ...
                if (FunctionName == null)
                {
                    ...
                    List<InnerExpression> expList = GetInnerExpressions(this.ParamsBody,
                        GetResourceDependency());
                    //foreach (InnerExpression ext in expList)
                    //{
                    //    Util.Debug(false, string.Concat("匿名表达式定义：", ext.TagDefinition));
                    //}
                    return ExecExpressions(expList);
                }
                #endregion
                ...
                //}
                //return Util.InvokeFunction(tInvoke, FunctionName, GetFunctionParams());

                throw new InvalidOperationException("*函数名称：" + FunctionName + "不支持，目前仅支持匿名函数！");
            }

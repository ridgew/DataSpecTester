...
namespace Gwsoft.DataSpec2
{
    ...
    /// <remarks>http://www.codeproject.com/KB/files/win32fileutilityclass.aspx</remarks>
    /// </summary>
    public class IniFile : IDisposable
    {
        ...
        /// <para>http://msdn2.microsoft.com/en-us/library/ms724348.aspx</para>
        /// </remarks>
        ...
        /// <summary>
        /// 保持Ini文件中特定定于区间键值的描述词典
        /// </summary>
        Dictionary<string, string> skCommentDict = new Dictionary<string, string>();

        string getCommentKey(string sectionName, string keyName)
        {
        ...
        /// 获取特定字段的描述说明
        /// </summary>
        public string GetComment(string sectionName, string keyName)
        {
            string key = getCommentKey(sectionName, keyName);
            if (skCommentDict.ContainsKey(key))
            {
                return skCommentDict[key].TrimStart(new char[] { '#' });
            }
            return null;
        }
            ...
            for (int i = 0; i < keyValuePairs.Length; ++i)
            {
                ...
                if (equalSignPos == -1)
                {
                    if (keyValuePairs[i].StartsWith("#"))
                    {
                        if (lastComment != null)
                        {
                            lastComment += Environment.NewLine + keyValuePairs[i];
                        }
                        else
                        {
                            lastComment = keyValuePairs[i];
                        }
                    }
                    else
                    {
                ...
                {
                    cmtKey = getCommentKey(sectionName, key);
                    if (skCommentDict.ContainsKey(cmtKey))
                    {
                        skCommentDict[cmtKey] = lastComment;
                    }
                    else
                    {
                        skCommentDict[cmtKey] = lastComment;
